...
export class Resizable {
    ...
    onMove(width, height, screenX, screenY, event) {
        const evt = getEvent(event);
    ...
    initResize(event, isSouth, isEast, isSouthEast) {
        if (isSouth) {
            this.resizingS = true;
        }
        if (isEast) {
    ...
    endResize(event) {
        ...
        const resizableEvent = { event: getEvent(event), width: this.newWidth, height: this.newHeight };
    ...
    resizeWidth(event) {
        ...
        if (this.resizingSE || this.resizingE) {
            if (overMinWidth && underMaxWidth) {
                ...
                const resizableEvent = { event, width: this.newWidth, height: this.newHeight, direction: 'horizontal' };
    ...
    resizeHeight(event) {
        ...
        if (this.resizingSE || this.resizingS) {
            if (overMinHeight && underMaxHeight) {
                ...
                const resizableEvent = { event, width: this.newWidth, height: this.newHeight, direction: 'vertical' };
                this.element.dispatchEvent(new CustomEvent('resizing', { detail: resizableEvent }));
            }
        }
    }
