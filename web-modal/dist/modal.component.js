...
export class ModalComponent extends HTMLElement {
    ...
    renderContent() {
        const header = this.querySelector('[select="app-modal-header"]');
        const body = this.querySelector('[select="app-modal-body"]');
        const footer = this.querySelector('[select="app-modal-footer"]');
        if (header) {
    ...
    focusLastModal() {
        const modal = this.parentElement.closest('.ui-modal');
        if (modal) {
            modal.focus();
        }
    }
